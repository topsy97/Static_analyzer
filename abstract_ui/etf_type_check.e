...
feature -- command
	type_check
		...
		model.pretty_print
			...
			if (model.type_check_result) then  --correct this
				model.default_update
				model.set_expression (model.get_pretty_text)
				model.set_report (model.get_pretty_text + model.report_is_type_correct)
				--model.evaluator
			...
			elseif model.get_pretty_text.has ('?') or model.get_pretty_text.has_substring ("nil") then
				...
				model.set_report (model.get_pretty_text + model.report_is_not_type_correct)
			end

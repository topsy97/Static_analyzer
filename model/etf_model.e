...
feature {ETF_MODEL_ACCESS} --{NONE} -- Initialization
	...
	make
		...
		do
			...
			create {SET_OP} enumerator.make
			enumerator_variable := false
...
--	get_pretty_exp: EXPRESSION
--		do
--			Result := printer.curr_expression
--		end
		...
		do
			...
			if stack.count >= 1 then
				...
				if attached {CURRENT_EXPRESSION} last_constant_pointer as constant_op then
					...
					-----------------------BINARY OPERATION----------------------
				if attached {BINARY_OPERATION} last_constant_pointer as stack_last_binary_op then
					...
					elseif stack_last_binary_op.right ~ curr then -- if right is '?'
						...
						loop
							...
							if attached {BINARY_OPERATION} stack.at (k) as c1_binary then
...
feature -- insert binary expressions
	...
	insert_binary_op_expression (bop: BINARY_OPERATION)
		local
			i, j, k: INTEGER
			curr: CURRENT_EXPRESSION
		do
			...
			elseif count > 0 then
				...
				if stack.count >= 1 then
					...
					if attached {BINARY_OPERATION} last_expression as stack_last_binary_op then
						...
						elseif stack_last_binary_op.right ~ curr then
							...
							loop
								...
								if attached {BINARY_OPERATION} stack.at (k) as previous_binary_op then
...
--insert_set_op_expression (uop: SET_OP)
--		local
--			i, j, k: INTEGER
--			curr: CURRENT_EXPRESSION
--			emp: EMPTY_EXPRESSION
...
--				tree.force (uop, count + 1)
--				tree.force (emp, count + 2)
--				across 1|..| uop.our_set.count as  p loop
--				tree.force (uop.our_set.as_array[k.item], k.item + 3)
--				stack.put_i_th (uop, k.item)
--				end
--			elseif count > 0 then
--					-- updating tree...
--				from
...
--				across 1|..| uop.our_set.count as  p loop
--							tree.put (uop.our_set.item, (2 * j) + 1)
--							end
